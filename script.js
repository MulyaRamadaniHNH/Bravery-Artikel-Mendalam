...
for (let i = 0; i < elementclose.length; i++) {
    elementclose[i].addEventListener('click', function() {
        ...
        let url = hashIndex !== -1 ? window.location.href.substring(0, hashIndex) : window.location.href;
        window.location = url;
    });
}
...
loadmore.addEventListener('click', (e) => {
    e.preventDefault();
    const elementList = [...document.querySelectorAll('.container-b1 li')];
    ...
    // Kembalikan posisi scroll setelah elemen baru ditampilkan
    setTimeout(function() {
